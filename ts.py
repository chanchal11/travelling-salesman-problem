...
graph_matrix_const = [[ 0,12,	10,	-1,	-1,	-1,	12],
[12,	0,	8,	12,	-1,	-1,	-1],
[10,	8,	0,	11,	3,	-1,	9],
[-1,	12,	11,	0,	11,	10,	-1],
[-1,	-1,	3,	11,	0,	6,	7],
...
def adjacent_list(vertex: int):
    adj_list = []
    for i in range(0,graph_matrix_const.__len__()):
        if graph_matrix_const[vertex][i] != 0 and graph_matrix_const[vertex][i] != -1:
            adj_list.append(i)
    return adj_list
  ...
  if check_if_all_visited() == True:
        ...
        if graph_matrix[sv][cv] < 0:
            ...
            if shortest_path == None:
                shortest_path = {'path': path, 'total_dist': total_dist + graph_matrix_const[cv][sv] }
                print(path)
                print(shortest_path)
            elif shortest_path['total_dist'] < total_dist + graph_matrix_const[cv][sv]:
                return
            elif shortest_path['total_dist'] > total_dist + graph_matrix_const[cv][sv]: 
                shortest_path = {'path': path, 'total_dist': total_dist + graph_matrix_const[cv][sv] }
            ...
            # total_dist = total_dist - graph_matrix_const[cv][sv]    
            # path = []
            return
...
#       return   	  
  ...
  for v in adjacent_list(cv):										  
    ...
    cal_path(sv,v,total_dist + graph_matrix_const[cv][v])
